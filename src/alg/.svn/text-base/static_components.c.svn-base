...
int64_t
parallel_shiloach_vishkin_components_with_types(struct stinger * S, int64_t nv,
                                      int64_t * component_map, int64_t * edge_types, int64_t num_edge_types)
{
  /* Initialize each vertex with its own component label in parallel */
  OMP ("omp parallel for")
    for (uint64_t i = 0; i < nv; i++) {
  ...
  while (1) {
    ...
    for(uint64_t t = 0; t < num_edge_types; t++) {
      STINGER_PARALLEL_FORALL_EDGES_BEGIN (S, edge_types[t]) {
	if (component_map[STINGER_EDGE_DEST] <
	    component_map[STINGER_EDGE_SOURCE]) {
	  component_map[STINGER_EDGE_SOURCE] = component_map[STINGER_EDGE_DEST];
	  changed++;
